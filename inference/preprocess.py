...
def extract_frames(filename,dataset,num_frames,model,image_size=(380,380)):
	video_path = os.path.dirname(filename)
	video_name = os.path.basename(filename)
	if dataset=='FF':
		folder_path = video_path+'/frames/'+video_name[:-4]
	elif dataset=='CDF':
		folder_path = video_path+'/frames/'+video_name[:-4]
	elif dataset=='DFDC':
		folder_path = video_path+'/frames/'+video_name[:-4]
	...
	if os.path.exists(folder_path):
		...
		for i in range(len(file)):
			if i<num_frames:
				img = cv2.resize(np.array(Image.open(file[i])), image_size).transpose(2, 0, 1)
				results.append(img)
				idx.append(int(os.path.basename(file[i])[:-4]))
			else:
				break
		return results, idx
	else:
	...
	for cnt_frame in range(frame_count_org): 
		...
		try:
			...
			temp = Image.fromarray(croppedfaces_temp[0].transpose(1,2,0))
			temp.save(folder_path+'/'+str(idx_list_temp[0]).zfill(5)+'.png')

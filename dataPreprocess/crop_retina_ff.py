...
def facecrop(model, org_path, num_frames=10):
	...
	for tmp_path in org_path:
		...
		for cnt_frame in range(min_frame_count):
			...
			save_path_ = os.path.dirname(tmp_path).replace('videos', 'frames/') + os.path.basename(tmp_path).replace(
				'.mp4', '/')
...
if __name__=='__main__':
	...
	parser.add_argument('-d', dest='dataset',
						choices=['DeepFakeDetection_original', 'DeepFakeDetection', 'FaceShifter', 'Face2Face',
								 ...
								 'YouTube-real', 'DFDC', 'DFDCP', 'ALL'], default='ALL')
	...
	if args.dataset == 'Original':
		...
		dataset_path.append(
			os.path.join(args.dataset_path, 'manipulated_sequences/{}/{}/videos/'.format(args.dataset, args.comp)))
	elif args.dataset == 'ALL':
		dataset_path.append(os.path.join(args.dataset_path, 'original_sequences/youtube/{}/videos/'.format(args.comp)))
		dataset_path.append(
			os.path.join(args.dataset_path, 'manipulated_sequences/{}/{}/videos/'.format('FaceShifter', args.comp)))
		dataset_path.append(
			os.path.join(args.dataset_path, 'manipulated_sequences/{}/{}/videos/'.format('Face2Face', args.comp)))
		dataset_path.append(
			os.path.join(args.dataset_path, 'manipulated_sequences/{}/{}/videos/'.format('Deepfakes', args.comp)))
		dataset_path.append(
			os.path.join(args.dataset_path, 'manipulated_sequences/{}/{}/videos/'.format('FaceSwap', args.comp)))
		dataset_path.append(
			os.path.join(args.dataset_path, 'manipulated_sequences/{}/{}/videos/'.format('NeuralTextures', args.comp)))
	else:
	...
	for i in tqdm(range(1000)):
		facecrop(model, video_list[:, i], num_frames=args.num_frames)

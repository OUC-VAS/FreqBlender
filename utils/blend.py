...
def get_blend_mask(mask):
	...
	mask=cv2.resize(mask,(size_w,size_h))#将图片scale到一个范围内的随即大小
	kernel_1=random.randrange(5,26,2) #此处random非numpy中的包
	kernel_1=(kernel_1,kernel_1)#高斯核大小
	kernel_2=random.randrange(5,26,2)
	kernel_2=(kernel_2,kernel_2)
	
	mask_blured = cv2.GaussianBlur(mask, kernel_1, 0)#使用高斯核进行图像模糊
	mask_blured = mask_blured/(mask_blured.max())
	mask_blured[mask_blured<1]=0#此处将所有小于1的值变为了0（掩码中存在着很多的1值）

	mask_blured = cv2.GaussianBlur(mask_blured, kernel_2, np.random.randint(5,46))
	mask_blured = mask_blured/(mask_blured.max())
	mask_blured = cv2.resize(mask_blured,(W,H))#进行过两次高斯核处理之后恢复到原本大小

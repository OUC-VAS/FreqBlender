...
class Mask():
    ...
    def __init__(self, landmarks, face, channels=4):
        ...
        self.mask = self.merge_mask(mask) #此处意义不大
...
class dfl_full(Mask):  # pylint: disable=invalid-name
    """ DFL facial mask """
    def build_mask(self):
        mask = np.zeros(self.face.shape[0:2] + (1, ), dtype=np.float32)#生成三维的mask数组

        nose_ridge = (self.landmarks[27:31], self.landmarks[33:34])#鼻子部分
        jaw = (self.landmarks[0:17],
               self.landmarks[48:68],
               self.landmarks[0:1],
               self.landmarks[8:9],
               self.landmarks[16:17])#下巴，下颚
        eyes = (self.landmarks[17:27],
                self.landmarks[0:1],
                self.landmarks[27:28],
                self.landmarks[16:17],
                self.landmarks[33:34])#眼睛部分
        parts = [jaw, nose_ridge, eyes]

        for item in parts:
            merged = np.concatenate(item)# 将mask的人脸每部分分别融合起来
